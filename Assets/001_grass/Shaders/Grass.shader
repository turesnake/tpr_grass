...
Shader "FT Scene/Grass"
{
    Properties
    {
        ...
        _ShadowColor("Shadow Color", Color) = (1,1,1,1)         // 草阴影颜色, 最好和地面的阴影颜色相近
        _DownColor("Down Color", Color) = (1,1,1,1)             // 草下段颜色, 最好和地面颜色相近
        _UpColor("Up Color", Color) = (1,1,1,1)                 // 草上段颜色
        _HighLightColor("hight light Color", Color) = (1,1,1,1) // 麦浪颜色
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                if(isUseWheatWave > 0)
                {
                    float2 wheatUV = GetWheatWaveUV( rootPosWS, wheatWaveDegree, wheatWaveGridSize, time, wheatWaveSpeed );
